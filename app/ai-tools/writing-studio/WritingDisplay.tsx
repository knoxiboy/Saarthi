...
import { AlignLeft, Check, Copy, Download, FileText, Loader2, RefreshCw, Wand2 } from "lucide-react"
...
export default function WritingDisplay({
    ...
    return (
        <div className="flex flex-col bg-white/2 border border-white/5 rounded-5xl p-7 md:p-9 shadow-2xl backdrop-blur-3xl animate-in fade-in slide-in-from-bottom-10 duration-1000 relative overflow-hidden h-fit max-h-[75vh] min-h-[600px]">
            ...
            <div className="flex-1 bg-white/2 rounded-4xl border border-white/5 overflow-y-auto flex flex-col shadow-inner relative group custom-scrollbar">
                <textarea
                    value={output}
                    onChange={(e) => setOutput(e.target.value)}
                    className="w-full h-full min-h-[400px] bg-transparent p-12 md:p-16 focus:outline-none text-slate-300 font-serif italic text-lg leading-relaxed resize-none selection:bg-blue-500/30 whitespace-pre-wrap wrap-break-word"
                />
            </div>
